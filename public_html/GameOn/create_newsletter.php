...
$page_title = 'Create A Newsletter';
...
if (isset($_POST['submitted'])) {
    ...
    if (empty($_POST['description'])) {
            $errors[] = 'You forgot to enter the description of the newsletter.';
    } else {
    ...
    if (empty($errors)) { // If everything's OK.
        $i = ($_POST['newsletter_id']);
        
        // Post the newsletter in the database...

        // Make the query:
        $q = "INSERT INTO newsletters (title, description) VALUES ('$t', '$d')";		
        $r = @mysqli_query ($dbc, $q); // Run the query.
        if ($r) { // If it ran OK.

                // Print a message:
                echo '<h1>Thank you!</h1>
        <p>Your newsletter was successfully posted!</p><p><br /></p>';	

        } else { // If it did not run OK.

                // Public message:
                echo '<h1>System Error</h1>
                <p class="error">Your newsletter could not be posted due to a system error. We apologize for any inconvenience.</p>'; 
...
<nav id="mainNav" class="navbar navbar-default navbar-fixed-top">
	...
	<h1><a>Register</a></h1>	
		...
		<form class="form-signin" role="form" action="create_newsletter.php" method="post">
    ...
    <p>Title: <input type="normal" class="form-control" placeholder="Title of Newsletter" required autofocus name="title" maxlength="200" value="<?php if (isset($_POST['title'])) echo $_POST['title']; ?>" /></p>
    <p>Description: <input type="normal" class="form-control" placeholder="Enter your text here." required name="description" maxlength="500" value="<?php if (isset($_POST['description'])) echo $_POST['description']; ?>" /></p>
	<p><button type="submit" name="submit" class="btn btn-sm btn-primary" />Create</button></p>
